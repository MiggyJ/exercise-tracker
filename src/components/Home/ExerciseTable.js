...
const ExerciseTable = ({ setActivity, startTimer }) => {
    ...
    useEffect(() => {
        ...
        const deleteFavorite = async () => {
            const res = await fetch('http://localhost:5000/user/favorite', {
                method: 'DELETE',
                headers: {
    ...
    const getFilteredExercises = async (tag) => {
        ...
        if (tag !== 'Suggested') {
            if (tag === 'Starred')
                tag = 'all'
            const res = await fetch(`http://localhost:5000/exercise/view/${tag}`)
            data = await res.json()
        } else {
            const res = await fetch('http://localhost:5000/exercise/random')
            data = await res.json()
        }
        return data
    }
